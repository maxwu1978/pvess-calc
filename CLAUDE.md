...
- `.claude/skills/pvess-review/` — 一键 rebuild + doctor + 栅格化 + 逐页 review
- `.claude/skills/pvess-visual-polish/` — **视觉打磨迭代** playbook（收口标准、碰撞模式、修法清单）
...
| `ee4_focuses_on_site_geometry` | K.13：EE-4 不得渲染 legacy `N×M grid` 抽象网格（K.13 已删除该路径）。正则 `\b\d+\s*[×x]\s*\d+\s+grid\b` 命中即 FAIL；正向：roof_sections 存在 → 实模块渲染；roof_sections 空 → 警告条 visible。|

**doctor 不覆盖的 gap**：text vs **wire/icon 几何** 碰撞。需肉眼复查（visual-polish skill §C/§D）。

**EE-4 三种渲染模式（K.13 契约）**：
- `wire_routing.routed=True` → 真实模块 + fire-offset hatch + Manhattan 导线 + leader-line 设备标注
- `has_face_anchors=True` 但无 `equipment_locations` → 真实模块 + fire-offset hatch（无导线、无 leader）
- 都没有（无 `roof_sections`）→ lot + house + setbacks + K.6 east-wall 设备 chip + 黄底 NOTE 警告条

**Stage B auto-anchor**（[`calc/site_layout.py`](src/pvess_calc/calc/site_layout.py)）：任何带 `roof_sections` 但没显式 `site_anchor` 的 yaml，引擎在 `engine.run()` 入口按 `azimuth_deg` 自动分配到 S/E/N/W 4 面墙并按 declared order 堆叠，gap=1 ft。Phoenix（2 面无 anchor）、Frisco（5 面显式 anchor）共用同一渲染路径。
...
| K.13 | EE-4 site-focused restructure（A=Frisco yaml K.11 数据 + B=auto-anchor + C=视觉打磨 + D=删除 legacy abstract grid + 扩大 aerial + doctor 守门）| `calc/site_layout.py` 新增, `permit/site_plan.py` 三模式分支, doctor `ee4_focuses_on_site_geometry` | ✅ done (2026-05-17) |
