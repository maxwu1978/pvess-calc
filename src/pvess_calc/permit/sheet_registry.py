...
def codes() -> tuple[str, ...]:
    ...
    return tuple(s.code for s in SHEET_REGISTRY)


def by_code(code: str) -> SheetSpec:
    """Look up a sheet by code; raises KeyError if not registered."""
    for s in SHEET_REGISTRY:
        if s.code == code:
            return s
    raise KeyError(f"Sheet code {code!r} not in SHEET_REGISTRY")


def cover_index_rows() -> list[tuple[str, str]]:
    ...
    actually emits.
    """
    return [(s.display_code, s.title) for s in SHEET_REGISTRY]
